...
export class GoalExtractor {
  ...
  static extractGoalFromConversation(
    ...
    if (Array.isArray(input)) {
      // Filter out AI messages that contain probing questions/templates if user messages exist
    ...
    if (projects.length === 0) {
      ...
      for (const line of lines) {
        ...
        // Match bullet points or numbered lists: e.g. "- Build auth", "1. Setup DB"
        const bulletMatch = trimmed.match(/^(?:[-*•]|\d+[.)])\s+(.*)/);
        if (bulletMatch && bulletMatch[1].trim().length > 2) {
          const sub = bulletMatch[1].trim();
          // Filter out if it's the title or definition of done label
      ...
      // If no bullet points, look for comma-separated milestones after "sub-projects:" or "milestones:"
      if (projects.length === 0) {
  ...
  static validateGoalImmutability(
    ...
    // 1. If original goal was locked, title and DoD cannot be modified without unlock
    ...
    // 2. Cannot delete active / in-progress projects from an active goal
    ...
    for (const origP of origProjects) {
      ...
      // If the project was in-progress or locked, it cannot be deleted
